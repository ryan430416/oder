...
async function render() {
  ...
  orders.forEach((o) => {
    ...
    <article class="card order-card" data-oid="${o.order_id}">
      ...
      <div class="muted">${sname(o.store_id)}</div>
      ...
      <ul class="item-list">${(o.items || [])
        .map((i) => `<li>${productLabel(i.product_id, i.product_name)} × ${i.quantity}</li>`)
        .join("")}</ul>
      ${can ? `<div class="row-actions"><button class="btn btn-danger" type="button" data-cancel="${o.order_id}">${t("cancel_order")}</button></div>` : ""}
    </article>`;
  });
  list.innerHTML = html;
}
